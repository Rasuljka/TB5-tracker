...
async def update_task(task_id: int, task: TaskIn):
    """Обновление задачи"""
    query = Task.__table__.update().where(Task.__table__.c.id == task_id).values(
        title=task.title,
        parent_task_id=task.parent_task_id,
        assignee_id=task.assignee_id,
        deadline=task.deadline,
...
async def get_important_tasks():
    ...
    query = (
        ...
        .where(
            exists().where(
                (Task.id == Task.parent_task_id) & (Task.status == "active")
            )
        )
    )
...
@app.get("/important_tasks_with_assignees/", response_model=List[Dict[str, Any]])
async def get_important_tasks_with_assignees():
    """Получает важные задачи, не взятые в работу, и от которых зависят другие задачи, взятые в работу"""
    ...
    for task in important_tasks:
        suitable_employees = [
            ...
            if emp["task_count"] <= min(emp["task_count"] for emp in available_employees) + 2
        ]
        result.append({
            "important_task": task,
            "deadline": task.deadline,
...
async def create_employee(employee: EmployeeIn):
    """Создание сотрудника"""
    async with database.transaction():
        query = Employee.__table__.insert().values(name=employee.name, job_title=employee.job_title)
        last_record_id = await database.execute(query)
...
async def update_employee(employee_id: int, employee: EmployeeIn):
    ...
    query = Employee.__table__.update().where(Employee.__table__.c.id == employee_id).values(
        name=employee.name,
